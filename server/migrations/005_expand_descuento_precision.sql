-- Migración 005: Ampliar precisión de columnas de descuento a NUMERIC(12,4)
-- Permite guardar porcentajes de descuento por marca y de pedidos con hasta 4 decimales

-- 1. Drop views in cascade
DROP VIEW IF EXISTS public.atc_detalles_pedidos_v CASCADE;
DROP VIEW IF EXISTS public.atc_pedidos_v CASCADE;

-- 2. Alter column types to NUMERIC(12,4)
ALTER TABLE "atc_migración".detalles_pedidos ALTER COLUMN descuento TYPE NUMERIC(12,4);
ALTER TABLE "atc_migración".pedidos ALTER COLUMN porcentaje_descuento TYPE NUMERIC(12,4);

-- 3. Recreate public.atc_pedidos_v
...
-- 4. Recreate trigger function for public.atc_pedidos_v
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.atc_pedidos_v TO anon, authenticated, service_role;

-- 5. Recreate public.atc_detalles_pedidos_v
...
-- 6. Recreate trigger function for public.atc_detalles_pedidos_v
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.atc_detalles_pedidos_v TO anon, authenticated, service_role;
